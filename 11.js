...
let takeNAdjacentElements = (n, arr) => {
    return {
        adj: _.first(arr, n),
        rest: _.tail(arr)
    };
...
let calculateHighestAdjacentProduct = (number, highest) => {
    if(_.isEmpty(number)) {
        return highest;
    }
    let {adj, rest} = takeNAdjacentElements(4, number);
    highest = _.max([prod(adj), highest]);
    return calculateHighestAdjacentProduct(rest, highest);
}

let transposeGrid = grid => _.range(0, grid.length * 2 - 1)
...
let checkRows = grid => grid.map(row => calculateHighestAdjacentProduct(row, 0));
let checkCols = grid => transposeGrid(grid).map(col => calculateHighestAdjacentProduct(col, 0));
let checkDiags = grid => getDiagonals(grid).map(diag => calculateHighestAdjacentProduct(diag, 0));

let validAnswer = 70600674;
let result = _.max(checkCols(grid).concat(checkRows(grid)).concat(checkDiags(grid)));
